 ...
 * Patch an asset
 */
async function assetUpdate(client, assetPatchPayload, assetId, options) {
    var _a;
    const input = {
        assetPatchPayload: assetPatchPayload,
        assetId: assetId,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => operations.UpdateAssetRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(200, operations.UpdateAssetResponse$inboundSchema, { key: "asset" }), M.fail(["4XX", "5XX"]), M.json("default", operations.UpdateAssetResponse$inboundSchema, {
        key: "error",
