...
export default function MapPage() {
  ...
  const handleStartStream = async ({ latitude, longitude, crisis }) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/livepeer/create`, {
        latitude,
        longitude,
        crisis,
      });
  ...
  const handleEndStream = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/livepeer/end/${streamData.id}`);
      setStreamData(null);
    } catch (error) {
